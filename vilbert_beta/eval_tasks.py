...
def eval_model(preds, labels):
    ea, corr_ea, sa, corr_sa, w, corr_w, af, corr_af = 0, 0, 0, 0, 0, 0, 0, 0

    for pred, label in zip(preds, labels):
        if pred["question_id"] >= 1000000 and pred["question_id"] <=  1000100:
            idx = -1
            max1 = -10000
            for i in range(4):
                if pred["answer"][i] > max1:
                    max1 = pred["answer"][i]
                    idx = i
            if idx == label:
                corr_ea += 1
        ...
        if pred["question_id"] >= 1000101 and pred["question_id"] <=  1000200:
            idx = -1
            max1 = -10000
            for i in range(4):
                if pred["answer"][i] > max1:
        ...
        if pred["question_id"] >= 1000201 and pred["question_id"] <=  1000287:
            idx = -1
            max1 = -10000
            for i in range(4):
                if pred["answer"][i] > max1:
        ...
        if pred["question_id"] >= 1000288 and pred["question_id"] <=  1000327:
            idx = -1
            max1 = -10000
            for i in range(4):
                if pred["answer"][i] > max1:
    ...
    print("east asia", corr_ea * 1. / ea)
    print("west", corr_w * 1. / w)
    print("south asia", corr_sa * 1. / sa)
    print("africa", corr_af * 1. / af)
    print("overall", (corr_ea+corr_w+corr_sa+corr_af) * 1. / (ea+w+sa+af))
...
def main():
    ...
    parser.add_argument(
        "--batch_size", default=16, type=int, help="what is the batch size?"
    )
    parser.add_argument(
    ...
    for task_id in task_ids:
        ...
        if "orig" not in task_cfg[task_id]['val_annotations_jsonpath']:
            with open(task_cfg[task_id]['val_annotations_jsonpath'], 'r') as f1:
                labels = [json.loads(s)["answer_label"] for s in f1]
            eval_model(results, labels)
