...
def _converId(img_id, orig_or_not="orig"):
...
def _load_annotationsQ_A(annotations_jsonpath, split):
    ...
    with open(annotations_jsonpath, 'rb') as f: # opening file in binary(rb) mode    
        for annotation in json_lines.reader(f):
            ...
            if "orig" in annotations_jsonpath:
                orig_or_not = "orig"
            else:
                orig_or_not = "not"
            img_id = _converId(annotation["img_id"], orig_or_not)
            anno_id = int(annotation["annot_id"].split('-')[1])
            entries.append(
                    {"objects": annotation["objects"], "question": question, 'answers':annotation["answer_choices"], "metadata_fn": annotation["metadata_fn"], 'target':ans_label, 'img_id':img_id, 'anno_id':anno_id}
            )
        ...
        for annotation in json_lines.reader(f):
            ...
            if split == 'test':
                ...
                for answer in annotation["answer_choices"]:
                    ...
                    anno_id = int(annotation["annot_id"].split('-')[1])
                    entries.append(
                            {"objects": annotation["objects"], "question": question, 'answers':annotation["rationale_choices"], "metadata_fn": annotation["metadata_fn"], 'target':ans_label, 'img_id':img_id}
                    )
            else:
                det_names = ""
                ...
                anno_id = int(annotation["annot_id"].split('-')[1])
                entries.append(
                        {"objects": annotation["objects"], "question": question, 'answers':annotation["rationale_choices"], "metadata_fn": annotation["metadata_fn"], 'target':ans_label, 'img_id':img_id, 'anno_id':anno_id}
                )
...
class VCRDataset(Dataset):
    def __init__(
        ...
        self._names = []
        with open('feature/VCR/unisex_names_table.csv') as csv_file:
    ...
    def tokenize(self):
        ...
        for entry in self._entries:
            metadata_fn = json.load(open(os.path.join('../X_VCR/vcr1images', entry["metadata_fn"]), 'r'))
            # det_names = metadata_fn["names"]
            det_names = entry["objects"]
